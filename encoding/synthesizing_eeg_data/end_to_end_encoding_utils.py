def load_images(args, idx_val):
	...
	from encoding_config import IMAGE_SET_DIR
	...
	img_dirs = os.path.join(IMAGE_SET_DIR, 'training_images')
	image_list = []
	...
	img_dirs = os.path.join(IMAGE_SET_DIR, 'test_images')
	image_list = []
	for root, dirs, files in os.walk(img_dirs):
		for file in files:
			if file.endswith(".jpg"):
...
def load_eeg_data(args, idx_val):
	...
	import numpy as np
	import torch
	from encoding_config import BRAIN_REGIONS, EEG_DATA_DIR

	selected_channels = BRAIN_REGIONS['occipital_parietal']
	### Load the EEG training data ###
	data_dir = os.path.join(EEG_DATA_DIR, 'sub-'+format(args.sub,'02'))
