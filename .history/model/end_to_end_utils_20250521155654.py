'''
@File    :   end_to_end_utils.py
@Time    :   2025/05/27 15:49:48
@Author  :   Weihao Xia 
@Version :   1.0
...
def load_images(args, idx_val):
	...
	img_dirs = os.path.join('/mnt/dataset0/ldy/4090_Workspace/4090_THINGS', 'images_set', 'training_images')
	image_list = []
	...
	img_dirs = os.path.join('/mnt/dataset0/ldy/4090_Workspace/4090_THINGS', 'images_set', 'test_images')
	image_list = []
	for root, dirs, files in os.walk(img_dirs):
		for file in files:
			if file.endswith(".jpg"):
...
def load_eeg_data(args, idx_val):
	...
	data = np.load(os.path.join('/mnt/dataset0/ldy/4090_Workspace/4090_THINGS', data_dir, training_file),
		allow_pickle=True)
